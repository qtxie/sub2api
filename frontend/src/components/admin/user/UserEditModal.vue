  ...
  >
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      ...
      <div class="flex items-start justify-between gap-4">
        <div>
          <label class="input-label">{{ t('admin.users.form.sessionStorage') }}</label>
          <p class="input-hint">{{ t('admin.users.form.sessionStorageHint') }}</p>
        </div>
        <Toggle v-model="form.session_storage_enabled" />
      </div>
...
const form = reactive({ email: '', password: '', username: '', notes: '', role: 'user', concurrency: 1, rpm_limit: 0, session_storage_enabled: false, customAttributes: {} as UserAttributeValuesMap })

watch(() => props.user, (u) => {
  if (u) {
    Object.assign(form, { email: u.email, password: '', username: u.username || '', notes: u.notes || '', role: u.role || 'user', concurrency: u.concurrency, rpm_limit: u.rpm_limit ?? 0, session_storage_enabled: u.session_storage_enabled ?? false, customAttributes: {} })
...
const handleUpdateUser = async () => {
  ...
  try {
    const data: any = { email: form.email, username: form.username, notes: form.notes, role: form.role, concurrency: form.concurrency, rpm_limit: form.rpm_limit, session_storage_enabled: form.session_storage_enabled }
