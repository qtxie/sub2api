-- Add SenseNova (商汤日日新, U1.5 生图) as a first-class platform.
--
...
--
-- Runs after 238_opencode_go_platform.sql. DROP ... IF EXISTS + 幂等守卫保证可重入；
-- 新约束是 238 的超集，必须同时保留 MiniMax 与 opencode_go。
...
ALTER TABLE user_platform_quotas
    ...
    CHECK (platform IN ('anthropic', 'openai', 'gemini', 'antigravity', 'grok',
                        'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'sensenova'));
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('sensenova' IN monitor_constraint_def) = 0 THEN
        ALTER TABLE channel_monitors
            DROP CONSTRAINT IF EXISTS channel_monitors_provider_check;
        ALTER TABLE channel_monitors
            ADD CONSTRAINT channel_monitors_provider_check
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'sensenova'));
    END IF;
    ...
    IF template_constraint_def IS NULL OR position('sensenova' IN template_constraint_def) = 0 THEN
