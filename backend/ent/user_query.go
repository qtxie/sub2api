...
func (_q *UserQuery) QueryChatConversations() *ChatConversationQuery {
...
func (_q *UserQuery) WithChatConversations(opts ...func(*ChatConversationQuery)) *UserQuery {
	query := (&ChatConversationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withChatConversations; query != nil {
		if err := _q.loadChatConversations(ctx, query, nodes,
			func(n *User) { n.Edges.ChatConversations = []*ChatConversation{} },
			func(n *User, e *ChatConversation) { n.Edges.ChatConversations = append(n.Edges.ChatConversations, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadChatConversations(ctx context.Context, query *ChatConversationQuery, nodes []*User, init func(*User), assign func(*User, *ChatConversation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ChatConversation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ChatConversationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
