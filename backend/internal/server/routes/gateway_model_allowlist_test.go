...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	// rootRoute helper：apiKeyAuth 之后、compositeTarget 之前；archiveCapture /
	// qcProbeRouting 为本地分支新增，同样位于鉴权之后、准入之前。
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), archiveCapture, qcProbeRouting, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)`))
	...
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), archiveCapture, qcProbeRouting, groupModelAllowlist, compositeTarget, requireGroupAnthropic)`))
