...
func RegisterGatewayRoutes(
	...
	// 之前，避免逐条路由手工维护链导致漏挂。archiveCapture / qcProbeRouting 为
	// 本地分支新增，同样位于鉴权之后、准入与合成改写之前。
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), archiveCapture, qcProbeRouting, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), archiveCapture, qcProbeRouting, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
