...
const apiKeyAuthSnapshotVersion = 22 // v22: merge upstream v20 (group long-context + model pricing fields) with session_storage_enabled in the auth projection
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		User: APIKeyAuthUserSnapshot{
			...
			SessionStorageEnabled:      apiKey.User.SessionStorageEnabled,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		User: &User{
			...
			SessionStorageEnabled:      snapshot.User.SessionStorageEnabled,
