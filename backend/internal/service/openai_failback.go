...
func (c *openAIFailbackController) selectionActionWithRetryAt(ctx context.Context, accountID int64, model string) (openAIFailbackAction, time.Time) {
	...
	case openAIFailbackPhaseCooldown:
		...
		// Cooldown elapsed: require a preflight probe before this account/model can
...
// admitExpiredCooldownToProbation moves an expired cooldown into probation after a
// successful preflight probe so the account/model can be scheduled again.
func (c *openAIFailbackController) admitExpiredCooldownToProbation(
	...
	if !fromProbe {
		// Switch-back admission must go through a real probe success path.
		return false
	}
	key, ok := openAIFailbackStateKey(accountID, model)
	if !ok {
