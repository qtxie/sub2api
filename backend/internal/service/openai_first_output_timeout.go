...
const openAIFirstOutputTimeoutBypassKey = "openai_first_output_timeout_bypass"

func markOpenAIFirstOutputTimeoutBypass(c *gin.Context) {
	if c != nil {
		c.Set(openAIFirstOutputTimeoutBypassKey, true)
	}
}

func openAIFirstOutputTimeoutBypassed(c *gin.Context) bool {
	if c == nil {
		return false
	}
	value, ok := c.Get(openAIFirstOutputTimeoutBypassKey)
	bypassed, _ := value.(bool)
	return ok && bypassed
}

func shouldBypassOpenAIFirstOutputTimeout(c *gin.Context, body []byte) bool {
	if isOpenAIResponsesCompactPath(c) {
		return true
	}
	if c == nil || c.Request == nil || !HasCompactionTriggerInInput(body) {
		return false
	}
