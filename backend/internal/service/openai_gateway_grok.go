...
func (s *OpenAIGatewayService) updateGrokUsageSnapshotWithRateLimit(ctx context.Context, account *Account, snapshot *xai.QuotaSnapshot, applyRateLimit bool) {
	...
	if applyRateLimit && hasActiveLimit && !account.IsPoolMode() {
...
func (s *OpenAIGatewayService) handleGrokAccountUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) {
	...
	if statusCode != http.StatusTooManyRequests {
		...
		if decision.ShouldCooldown && decision.Class != GrokFailureNone &&
			decision.Class != GrokFailureRateLimit && !deferGatewayCooldown {
