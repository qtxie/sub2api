...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if err := NormalizeOpenAIAPIKeyListCredentials(input.Platform, input.Type, input.Credentials); err != nil {
		return nil, err
	}
	if err := NormalizeHeaderOverrideCredentials(input.Credentials); err != nil {
		return nil, err
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if account.IsCredentialShadow() && input.Credentials != nil {
		...
		if err := NormalizeOpenAIAPIKeyListCredentials(account.Platform, account.Type, account.Credentials); err != nil {
			return nil, err
		}
		// 校验并规范化请求头覆写配置（header 名小写化、格式检查）
