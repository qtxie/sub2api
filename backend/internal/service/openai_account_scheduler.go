...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	platform := PlatformOpenAI
	if len(platformOverride) > 0 && strings.TrimSpace(platformOverride[0]) != "" {
		platform = platformOverride[0]
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithFailback(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	...
	return s.selectAccountWithFailback(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}

func (s *OpenAIGatewayService) selectAccountWithFailback(
	ctx context.Context,
	groupID *int64,
	previousResponseID string,
	sessionHash string,
...
// ReportOpenAIAccountScheduleResult records the outcome of a scheduled request.
// Failures additionally publish a gateway notification for operator diagnostics.
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResult(account *Account, model string, success bool, firstTokenMs *int, observedErr ...error) bool {
	...
	return s.reportOpenAIAccountScheduleResult(account, model, success, firstTokenMs, observedErr...)
}
...
// identity used elsewhere in gateway diagnostics. An optional observedErr is
// forwarded to API-key health observation the same way
// ReportOpenAIAccountScheduleResult does.
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleFailure(account *Account, user *User, model string, statusCode int, observedErr ...error) {
	if account == nil {
		return
	}
	...
	_ = s.reportOpenAIAccountScheduleResult(account, model, false, nil, observedErr...)
}
