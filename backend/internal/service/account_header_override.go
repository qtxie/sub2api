...
func (a *Account) ApplyHeaderOverrides(h http.Header) {
	...
	// Capture client original User-Agent before any override mutates headers so
	// smart multi-slot user-agent values can map against the inbound client.
	clientUA := h.Get("User-Agent")
	...
	for name, value := range overrides {
		if name == "user-agent" {
			value = ResolveSmartUserAgent(value, clientUA)
		}
