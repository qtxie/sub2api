...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if account != nil && account.Type == AccountTypeOAuth {
		if rejectReason := detectOpenAIPassthroughInstructionsRejectReason(reqModel, body); rejectReason != "" {
			...
			writeOpenAICompactAwareJSONError(c, http.StatusForbidden, "forbidden_error", rejectMsg, nil)
	...
	if imageIntent && !GroupAllowsImageGeneration(apiKeyGroup(apiKey)) {
		...
		writeOpenAICompactAwareJSONError(c, http.StatusForbidden, "permission_error", ImageGenerationPermissionMessage(), nil)
	...
	if imageIntent {
		...
		if imageCfgErr != nil {
			...
			writeOpenAICompactAwareJSONError(c, http.StatusBadRequest, "invalid_request_error", imageCfgErr.Error(), gin.H{"param": "size"})
	...
	var headerLatency time.Duration
	for {
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if account.Type == AccountTypeOAuth {
		...
		if retrySession := openAIFirstOutputRetrySession(c); retrySession != "" {
			clientSessionID = retrySession
			clientConversationID = retrySession
		}
		if isOpenAIResponsesCompactPath(c) {
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	directFlushPending := false
	flushDirectOutput := func() {
		if clientDisconnected || !directFlushPending {
			return
		}
		flusher.Flush()
		directFlushPending = false
	}
	defer flushDirectOutput()
	...
	flushPendingBoundary := func() error {
		if len(pendingLines) == 0 || (!semanticOutputCommitted && !pendingSemantic) {
			return nil
		}
		...
		if !semanticOutputCommitted && pendingSemantic {
	...
	for documentScanner.Scan() {
		...
		if !clientDisconnected {
			if !OpenAIPreOutputEnabled(c) {
				if !clientOutputStarted && !lineStartsClientOutput && !lineIsDone {
					pendingLines = append(pendingLines, line)
					continue
				}
				writeDirectLine := func(value string) bool {
					if _, writeErr := fmt.Fprintln(w, value); writeErr != nil {
						markClientDisconnected()
						return false
					}
					directFlushPending = true
					return true
				}
				for _, pendingLine := range pendingLines {
					if !writeDirectLine(pendingLine) {
						break
					}
				}
				pendingLines = nil
				if !clientDisconnected && writeDirectLine(line) {
					clientOutputStarted = true
			...
			pendingLines = append(pendingLines, line)
			if lineStartsClientOutput && !lineIsDone {
				pendingSemantic = true
			}
			if line == "" {
				if flushErr := flushPendingBoundary(); flushErr != nil {
