...
func TestHandleGrokAccountUpstreamError5xxRespectsPoolMode(t *testing.T) {
	...
	t.Run("non-pool mode keeps short cooldown for bad gateway", func(t *testing.T) {
		...
		require.WithinDuration(t, before.Add(time.Minute), repo.lastTempUnschedUntil, time.Second)
	})

	t.Run("non-pool mode keeps two minute cooldown for other 5xx", func(t *testing.T) {
		account := &Account{ID: 613, Platform: PlatformGrok, Type: AccountTypeAPIKey}
		repo := &grokQuotaAccountRepo{}
		svc := &OpenAIGatewayService{accountRepo: repo}
		before := time.Now()
