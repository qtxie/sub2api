...
func TestAccountFromServiceShallow_RedactsSensitiveCredentials(t *testing.T) {
	src := &service.Account{
		...
		Credentials: map[string]any{
			...
			"id_token":      "id-secret",
			"api_key":       "sk-secret",
			"api_key_list":  []any{"sk-fallback-secret"},
	...
	require.NotContains(t, got.Credentials, "api_key_list")
	// 非敏感键保留
	...
	require.True(t, got.CredentialsStatus["has_api_key_list"])
	...
	require.NotContains(t, string(raw), "sk-fallback-secret")
