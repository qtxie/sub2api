...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		if !errors.As(err, &failoverErr) {
			h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, requestedModel, false, result), http.StatusBadGateway, err)
		...
		h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, requestedModel, false, result), failoverErr.StatusCode, err)
		if c.Writer.Size() != writerSizeBeforeForward {
