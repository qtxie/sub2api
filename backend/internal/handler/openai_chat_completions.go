			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.ShouldReportAccountScheduleFailure() {
						h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, reqModel, false, nil), failoverErr.StatusCode, err)
					}
				...
				h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, reqModel, false, nil), http.StatusBadGateway, err)
