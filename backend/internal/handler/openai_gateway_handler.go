			...
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.ShouldReportAccountScheduleFailure() {
						h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, forwardModel, requireCompact, nil), failoverErr.StatusCode, err)
					}
				...
				h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, forwardModel, requireCompact, result), http.StatusBadGateway, err)
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.ShouldReportAccountScheduleFailure() {
						h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, currentRoutingModel, false, nil), failoverErr.StatusCode, err)
					}
					if !failoverErr.ShouldRetryNextAccount() {
				...
				h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, currentRoutingModel, false, result), http.StatusBadGateway, err)
		...
		scheduleSuccess := openAIForwardSucceededForScheduling(result)
		if !scheduleSuccess {
			h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, currentRoutingModel, false, result), http.StatusBadGateway)
		} else if result != nil {
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	handleWSFailover := func(account *service.Account, failoverErr *service.UpstreamFailoverError) bool {
		...
		if failoverErr.ShouldReportAccountScheduleFailure() {
			h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, wsForwardModel, false, nil), failoverErr.StatusCode, failoverErr)
		}
		releaseAccountSlot()
		if !failoverErr.ShouldRetryNextAccount() {
	...
	for {
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				if openAIForwardSucceededForScheduling(result) {
					h.gatewayService.ReportOpenAIAccountScheduleResult(account, scheduleModel, true, result.FirstTokenMs)
				} else {
					h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, scheduleModel, http.StatusBadGateway)
				}
				inboundEndpoint := GetInboundEndpoint(c)
		...
		for {
			...
			if shouldReportOpenAIWSProxyAccountFailure(err) {
				h.gatewayService.ReportOpenAIAccountScheduleFailure(account, apiKey.User, openAIAccountScheduleModel(c, account, wsForwardModel, false, nil), http.StatusBadGateway, err)
			}
			closeStatus, closeReason := summarizeWSCloseErrorForLog(err)
