...
func TestSameAccountRetryDelayFor(t *testing.T) {
	previousDelay := sameAccountRetryDelay
	sameAccountRetryDelay = defaultSameAccountRetryDelay
	t.Cleanup(func() { sameAccountRetryDelay = previousDelay })
...
func TestHandleFailoverError_SameAccountRetry(t *testing.T) {
	t.Run("生产重试间隔为500毫秒", func(t *testing.T) {
		require.Equal(t, 500*time.Millisecond, defaultSameAccountRetryDelay)
	})
